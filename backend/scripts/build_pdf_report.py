...
def _normalize_module_key(s: str) -> str:
    """
    Normalize incoming module key to SUPPORTED_KEYS style:
      - lower
      - strip spaces
      - convert '-' to '_'
...
def _get_first_str(d: dict, keys):
    """
    Return first non-empty string value for any key in keys from dict d.
    """
...
def _pick_module_key(cli_module_key: str, meta: dict, vol: dict) -> str:
    ...
    Priority:
      ...
      2) meta.json moduleKey/module_key
      3) volume_results.json moduleKey/module_key (also nested in result)
      4) default
    """
    # 1) CLI
    mk_cli_raw = (cli_module_key or "")
    mk_cli = _normalize_module_key(mk_cli_raw)
    if mk_cli:
        if mk_cli in SUPPORTED_KEYS:
            print(f"[INFO] moduleKey resolved from CLI: {mk_cli}")
            return mk_cli
        else:
            print(f"[WARN] CLI moduleKey not recognized: {mk_cli_raw} -> {mk_cli} (will fallback)")

    # 2) meta.json (support both moduleKey and module_key)
    mk_meta_raw = _get_first_str(meta, ["moduleKey", "module_key", "module"])
    mk_meta = _normalize_module_key(mk_meta_raw)
    if mk_meta in SUPPORTED_KEYS:
        print(f"[INFO] moduleKey resolved from meta.json: {mk_meta}")
        return mk_meta

    # 3) volume_results.json (support both moduleKey and module_key)
    mk_vol_raw = _get_first_str(vol, ["moduleKey", "module_key", "module"])
    mk_vol = _normalize_module_key(mk_vol_raw)
    if mk_vol in SUPPORTED_KEYS:
        print(f"[INFO] moduleKey resolved from volume_results.json: {mk_vol}")
        return mk_vol

    # 3b) sometimes moduleKey might be nested under vol["result"]
    if isinstance(vol, dict) and isinstance(vol.get("result"), dict):
        mk_vol2_raw = _get_first_str(vol["result"], ["moduleKey", "module_key", "module"])
        mk_vol2 = _normalize_module_key(mk_vol2_raw)
        if mk_vol2 in SUPPORTED_KEYS:
            print(f"[INFO] moduleKey resolved from volume_results.json.result: {mk_vol2}")
            return mk_vol2

    # 4) default
    print("[WARN] moduleKey not found in CLI/meta/vol -> using default circular_approx1")
    return "circular_approx1"
