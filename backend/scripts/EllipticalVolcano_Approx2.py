...
class VolumeAnalysisApp(QMainWindow):
    ...
    def calculate_results(self):
        try:
            ...
            self.V = ((np.pi * self.h_km) / 3) * ((self.area_caldera / np.pi) + (self.area_base / np.pi) + np.sqrt((self.area_caldera / np.pi) * (self.area_base / np.pi)))
    ...
    def download_results(self):
        ...
        if file_path:
            if not file_path.lower().endswith('.pdf'):
                file_path += '.pdf'
            try:
                # Definisci il titolo personalizzato per l'approssimazione 2
                title = "Calculation Results - Elliptical Base, Approximation Type 2"

                # Chiama la funzione generate_pdf dal modulo pdf_generator passando il titolo
                pdf_generator.generate_pdf(
                    file_path=file_path,
                    results_list=self.results_list,
                    title=title  # Passaggio del titolo personalizzato
                )
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python EllipticalVolcano_Approx2.py <dem_file_path>")
        sys.exit(1)
