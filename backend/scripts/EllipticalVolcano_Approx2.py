...
def _load_analysis_manifest_with_fallback(out_dir: str):
    """
    1) usa manifest in out_dir
    2) se assente, cerca il manifest più recente in outputs/*/analysis_images.json
    3) se ancora nulla, fallback ai file triplet_*.png in out_dir
...
class VolumeAnalysisApp(QMainWindow):
    def __init__(self, dem, original_file_name="Unknown"):
        ...
        self.calculate_results()  # Calcola prima di creare i widget
        self.initUI()
    ...
    def calculate_results(self):
        try:
            ...
            # Aree
            self.area_base = calculate_area(self.base_contour, pixel_size) * 1e-6  # m² -> km²
            self.area_caldera = calculate_area(self.caldera_contour, pixel_size) * 1e-6  # m² -> km²

            # Volume totale: tronco con basi ellittiche (usa aree)
            self.h = np.max(self.dem)
            self.h_km = self.h * 1e-3
            self.V = ((np.pi * self.h_km) / 3) * (
                (self.area_caldera / np.pi) + (self.area_base / np.pi) + np.sqrt((self.area_caldera / np.pi) * (self.area_base / np.pi))
            )

            # Volume caldera (cyl. con base ellittica)
            self.v_caldera = np.pi * (self.area_caldera / np.pi) * (self.distance_caldera_km / 2)

            # Volume effettivo
            self.v_volcano = self.V - self.v_caldera
            ...
            self.results_text = (
                ...
                f"Total volume of the volcanic edifice: {self.V:.2f} km³\n"
    ...
    def _collect_report_images(self):
        ...
        # overview
        overview_path = os.path.join(self.out_dir, "elliptical_approx2_overview.png")
        if os.path.exists(overview_path):
            paths.append(overview_path)
            captions.append("Elliptical Approximation 2 — Overview")
    ...
    def download_results(self):
        # Salva i risultati in un file PDF utilizzando pdf_generator
