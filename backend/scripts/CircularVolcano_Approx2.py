...
# ——————————————————————————————————————————————————————————
# Approx2: mantiene i calcoli esistenti (NON TOCCATI).
# Headless/Docker:
...
def _find_manifest():
    ...
    Fallback: manifest più recente in ${OUTPUTS_DIR}/*/analysis_images.json (se esiste).
    """
...
def run_volume_analysis(
    ...
    *,
    ...
    - calcola (LOGICA SCIENTIFICA INVARIATA)
    ...
    # --- calcoli invariati (copiati 1:1 dalla classe) ---
    ...
    distance_pixel_base = distance_between_points(
        base_point1[0], base_point1[1], base_point2[0], base_point2[1]
    )
    ...
    # Volumi (Approx2: caldera cilindro h=r)  ✅ INVARIATO
    ...
    v_km3 = float(v * 1e-9)
    r2 = float(R2 * 1e-3)     # km
    p = float(r2)             # km (h=r)  ✅ INVARIATO
    v_caldera = float(np.pi * (r2**2) * p)  # km^3 ✅ INVARIATO
...
if not HEADLESS:
    class VolumeAnalysisApp(QMainWindow):
        def __init__(self, dem, original_file_name="Unknown"):
            ...
            # outputs context (GUI: non obbligatorio scrivere file)
        ...
        # ----- doppietta finale: usa funzione core -----
...
def main(argv=None):
    ...
    # original_file_name non serve per headless, ma lasciamo compat
    ...
    if HEADLESS:
        # [PATCH] Headless: scrive outputs + stampa UNA riga JSON finale
