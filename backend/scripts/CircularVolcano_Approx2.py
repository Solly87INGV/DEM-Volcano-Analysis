...
class VolumeAnalysisApp(QMainWindow):
    ...
    def calculate_results(self):
        try:
            ...
            self.p = self.r2
            self.v_caldera = np.pi * (self.r2**2) * self.p  # Volume of the caldera
            self.v_volcano = self.v_km3 - self.v_caldera  # Effective volume
    ...
    def update_display(self):
        ...
        ax1.set_xlabel("")
        ax1.set_ylabel("")
        ax1.axis('on')  # Mostra assi

        # Grafico 2: Opposite Points of the Volcano Base
        ...
        ax2.set_xlabel("")
        ax2.set_ylabel("")
        ax2.axis('on')  # Mostra assi

        # Grafico 3: Opposite Maximum Slope Points on the Caldera
        ...
        ax3.set_xlabel("")
        ax3.set_ylabel("")
        ax3.axis('on')  # Mostra assi

        # Legenda per il Grafico 2
        legend_ax2 = fig.add_subplot(gs[1, 1])
        legend_ax2.axis('off')  # Nascondi assi
        ...
        # Legenda per il Grafico 3
        legend_ax3 = fig.add_subplot(gs[1, 2])
        legend_ax3.axis('off')  # Nascondi assi
        ...
        legend_ax1.axis('off')  # Nascondi assi
        ...
        desc_ax1.axis('off')  # Nascondi assi
        # Aggiungi un placeholder o lascia vuoto per simmetria
        desc_ax1.text(0, 0, "", fontsize=10, ha='left', va='center')

        # Descrizione per il Grafico 2 con bordi e allineamento centrale
        desc_ax2 = fig.add_subplot(gs[2, 1])
        desc_ax2.axis('off')  # Nascondi assi
        desc_ax2.text(
            0.5, 1.35, 
            self.description_base, 
            fontsize=10, 
        ...
        # Descrizione per il Grafico 3 con bordi e allineamento centrale
        desc_ax3 = fig.add_subplot(gs[2, 2])
        desc_ax3.axis('off')  # Nascondi assi
        desc_ax3.text(
            0.5, 1.5, 
            self.description_slope, 
            fontsize=10, 
        ...
        # Adatta il layout per fare spazio alle legende e alle descrizioni
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python CircularVolcano_Approx2.py <dem_file_path>")
        sys.exit(1)
