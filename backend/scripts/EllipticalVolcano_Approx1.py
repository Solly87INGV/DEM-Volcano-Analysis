...
# ——————————————————————————————————————————————————————————
# Approx1 (ellittico): calcoli invariati.
# Report:
...
#  - export automatico silenzioso in __init__
...
class VolumeAnalysisApp(QMainWindow):
    def __init__(self, dem, transform, meta=None, original_file_name="Unknown"):
        ...
        # salva overview + emetti payload JSON (non blocca la GUI)
        try:
    ...
    def update_display(self):
        ...
        # Piccolo margine extra a destra per non tagliare tick/label
        ...
        # NIENTE fig.subplots_adjust(...): lasciamo gestire a constrained_layout
        self.canvas.draw()
    ...
    def _build_metrics_dict(self) -> dict:
        ...
        # Volumi: edifice in m³ (self.v); caldera nel tuo Approx1 è già in km³ (self.v_caldera)
        ...
        metrics = {
            ...
            "volume_model": {
                ...
                # mantengo la logica: caldera dal tuo schema originale (non lo cambio)
                "caldera_model": "approx1_original",
            },
